...
try:
    ...
    # SemLock kinds (mirror CPython constants)
    _SEM_LOCK_SEMAPHORE = 0
    _SEM_LOCK_RECURSIVE_MUTEX = 1
    ...
    class _SemLock:
        ...
        def acquire(self, block=True, timeout=None):
            if timeout is not None:
                return self._lock.acquire(blocking=block, timeout=timeout)
            return self._lock.acquire(blocking=block)
        ...
        def _is_zero(self):
            # Stub: assumes semaphore is not exhausted.
            return False

        def _get_value(self):
            # Stub: returns 1 (semaphore available); exact counting is not
            # required for the read-only spack operations run in the browser.
            return 1
    ...
    # Also ensure the high-level multiprocessing package can be imported
    # cleanly by pre-populating multiprocessing.synchronize with stubs if
